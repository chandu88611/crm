...
import {  useAssignEnquiryMutation, useLazyNewEnqPaginationQuery, useLazyRequestEnqsQuery, useNewEnqSearchMutation } from "../../../services/api";
import { Table } from "react-bootstrap"
import Swal from "sweetalert2";
import Loader from "../Loader";

/* eslint-disable no-unused-vars */
export default function NewEnquiriesTable({ type, data, enquirySelected, refetch }) {
    ...
    if (tableData?.data?.enqs?.data) {
      if (colType == "string") {
        ...
        setTableData(updatedTableData);
         if (updatedTableData?.data?.staffs) {
           setStaffList(updatedTableData?.data?.staffs);
         }
  ...
  const [assignEnq, {data:assignRes, isLoading:assignLoading}] = useAssignEnquiryMutation()

  const handleAssignEnquiries = async() => {
    const response = await assignEnq({ staff_id: staff.value, enq_ids: checkedIds })
    reset()
  ...
  const [requestEnqs, { data: reqResp, isLoading: requestEnqLoading }] =
    useLazyRequestEnqsQuery();

  const handleRequestEnquiries = async () => {
    const response = await requestEnqs();
    if (response?.data?.status) {
      refetch();
    }
  };
  ...
  return (
    <>
      {searchLoading || (loaderState && <Loader />)}
                    ...
                    label: (
                      <div className="flex">
                       ...
                       {staff?.name}
                        ...
                        {staff?.today_login=="present" ? (
       <div class="ri-checkbox-blank-circle-fill text-success align-middle me-2 w-fit !ml-auto"></div>
                        ) : (
                          <div class="ri-checkbox-blank-circle-fill text-danger align-middle me-2 w-fit !ml-auto"></div>
                        )}
                      </div>
                    ),
                  }))}
              ...
              <button
                ...
                // disabled={true}
                onClick={() => handleRequestEnquiries()}
              >
        ...
        {type == "new" && (
          <div className="card-body border border-dashed border-end-0 border-start-0">
            ...
            <div className="row g-3">
              <div className="col-xxl-10 col-lg-8 col-sm-10">
              ...
              {tableData?.data?.enqs?.data?.length > 0 &&
                tableData?.data?.enqs?.data?.map((enq, ind) => (
                  <tr key={ind}>
                    ...
                    <td>{enq?.phone ? enq?.phone : "NA"}</td>
                    <td>{enq?.city ? enq?.city : "NA"}</td>
                    <td>{enq?.state ? enq?.state : "NA"}</td>
                    <td>
                    ...
                    <td>
                      <span>{enq?.ip_address ? enq?.ip_address : "NA"}</span>
                    </td>
                    <td>
            ...
            <div className="">
              ...
              >
                <ul className="pagination">
                  ...
                  {tableData?.data?.enqs?.total > 0 &&
                    tableData?.data?.enqs?.links?.map((page, ind) => (
                      <li
