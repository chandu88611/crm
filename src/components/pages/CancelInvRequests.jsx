...
export default function InvoiceCancelRequest() {
  ...
  const [fetchCancelRequests, { data: transferListResp, isLoading }] = useLazyInvCancelRequestsQuery();

  const fetchTransferData = async () => {
    const response = await fetchCancelRequests({
      type: activeTab == 1 ? "pending": activeTab == 2 ? "accepted" : "rejected"
    });
  ...
  const handleRejectRequest = async (id,type) => {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text: `You are about to ${type} this invoice cancel request. This process cannot be undone.`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
            ...
            <div className="row">
              <div className="col-12">
                <div className="page-title-box d-sm-flex align-items-left justify-content-between">
                  <p className="h5">Invoice Cancel Requests</p>
                                      ...
                                      transferData?.requests?.data?.map(
                                        (req, ind) => (
                                          ...
                                          >
                                            ...
                                            <td className="name text-center cursor-default text-capitalize">
                                              {req?.staff?.name}
                                            </td>
                                            <td className="name text-center cursor-default text-capitalize !bg-gradient-to-r from-rose-400 to-red-500 bg-clip-text text-transparent">
                                              {req?.req_staff?.name}
                                            </td>
